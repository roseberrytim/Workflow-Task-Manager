Ext.define('Custom.extension.Word', {
	...
	constructor: function(config) {
		...
		me._private.plugs.type.msoffice.makeOfficeGenerator('word', 'document', {});
		
		me.on('clearData', function() {
			me.data.length = 0;
		});

		me._private.plugs.type.msoffice.addInfoType('dc:title', '', 'title', 'setDocTitle');
		me._private.plugs.type.msoffice.addInfoType('dc:subject', '', 'subject', 'setDocSubject');
		me._private.plugs.type.msoffice.addInfoType('cp:keywords', '', 'keywords', 'setDocKeywords');
		me._private.plugs.type.msoffice.addInfoType('dc:description', '', 'description', 'setDescription');
		...
		me._private.type.msoffice.rels_app.push({
		...
		me.data = []; // All the data will be placed here.

		me._private.plugs.intAddAnyResourceToParse('docProps\\app.xml', 'buffer', null, Ext.bind(me.cbMakeDocxApp, me), true);
		me._private.plugs.intAddAnyResourceToParse('word\\fontTable.xml', 'buffer', null, Ext.bind(me.cbMakeDocxFontsTable, me), true);
		me._private.plugs.intAddAnyResourceToParse('word\\settings.xml', 'buffer', null, Ext.bind(me.cbMakeDocxSettings, me), true);
		me._private.plugs.intAddAnyResourceToParse('word\\webSettings.xml', 'buffer', null, Ext.bind(me.cbMakeDocxWeb, me), true);
		me._private.plugs.intAddAnyResourceToParse('word\\styles.xml', 'buffer', null, Ext.bind(me.cbMakeDocxStyles, me), true);
		me._private.plugs.intAddAnyResourceToParse('word\\document.xml', 'buffer', me, Ext.bind(me.cbMakeDocxDocument, me), true);

		me._private.plugs.intAddAnyResourceToParse('word\\_rels\\document.xml.rels', 'buffer', me._private.type.msoffice.rels_app, me.generateRelsXMLMarkup, true);
...
/**
	...
	createP: function(options) {
		...
		newP.addImage = function(image_path, opt, image_format_type) {
			...
			var image_id = me._private.type.msoffice.src_files_list.indexOf(image_path);
			var image_rel_id = -1;

			if (image_id >= 0) {
				for (var j = 0, total_size_j = me._private.type.msoffice.rels_app.length; j < total_size_j; j++) {
					if (me._private.type.msoffice.rels_app[j].target == ('media/image' + (image_id + 1) + '.' + image_type)) {
						image_rel_id = j + 1;
					} // Endif.
				} // Endif.

			} else {
				image_id = me._private.type.msoffice.src_files_list.length;
				me._private.type.msoffice.src_files_list[image_id] = image_path;
				me._private.plugs.intAddAnyResourceToParse('word\\media\\image' + (image_id + 1) + '.' + image_type, (typeof image_path == 'string') ? 'file' : 'stream', image_path, null, false);
			} // Endif.

			if (image_rel_id == -1) {
				image_rel_id = me._private.type.msoffice.rels_app.length + 1;
	 ...
	 *  @param[in] options ???.
	 */
	createListOfDots: function(options) {
		var newP = this.createP(options);
	 ...
	 *  @param[in] options ???.
	 */
	createListOfNumbers: function(options) {
		var newP = this.createP(options);
	...
	putPageBreak: function() {
		...
		this.data[this.data.length] = newP;
		return newP;
	},
  ...
  /**
	 ...
	 *  @return Text string.
	 */
	cbMakeDocxWeb: function (data) {
		return this.generateXMLHeaderMarkup(data) + '<w:webSettings xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:optimizeForBrowser/></w:webSettings>';
	},
	 ...
	 *  @return Text string.
	 */
	cbMakeDocxApp: function (data) {
		var userName = this.options.creator || 'officegen';
		var outString = this.generateXMLHeaderMarkup(data) + '<Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties" xmlns:vt="http://schemas.openxmlformats.org/officeDocument/2006/docPropsVTypes"><Template>Normal.dotm</Template><TotalTime>1</TotalTime><Pages>1</Pages><Words>0</Words><Characters>0</Characters><Application>Microsoft Office Word</Application><DocSecurity>0</DocSecurity><Lines>1</Lines><Paragraphs>1</Paragraphs><ScaleCrop>false</ScaleCrop><Company>' + userName + '</Company><LinksUpToDate>false</LinksUpToDate><CharactersWithSpaces>0</CharactersWithSpaces><SharedDoc>false</SharedDoc><HyperlinksChanged>false</HyperlinksChanged><AppVersion>12.0000</AppVersion></Properties>';
		return outString;
	},
			...
			for (var j = 0, total_size_j = objs_list[i].data.length; j < total_size_j; j++) {
				if (objs_list[i].data[j]) {
					...
					if (objs_list[i].data[j].text) {
						...
						outString += '<w:t' + tExtra + '>' + Ext.String.htmlEncode(objs_list[i].data[j].text) + '</w:t></w:r>';
